...
That narrow rule supports both a later convenience install and two concurrent
first installs: under the project lock, distinct dependencies are merged into
one deterministic generated manifest. A conflicting requirement leaves the
file unchanged and directs the caller to `zed add`.
...
embedded flags-2-env contract expose the same canonical flag spelling and
aliases. During the migration window, process startup bridges the canonical
environment variable into the legacy embedded environment key before
flags2env and Clap parse configuration; an explicit CLI option retains
precedence over inherited environment configuration.
...
does not pollute the project and is independent of the global content-store
lock. It serializes manifest creation/merge while allowing the existing
installer to retain ownership of store and project-tree transactions.
...
| Conflicting generated requirement | fail with no silent winner or file replacement |
